...
class PortalDataMapper extends BaseDataMapper_1.default {
    ...
    async simulateAttest(portalAddress, attestationPayload, validationPayloads, value = 0n) {
    ...
    async attest(portalAddress, attestationPayload, validationPayloads, waitForConfirmation = false, value = 0n) {
    ...
    async simulatePortalContract(portalAddress, functionName, args, value = 0n) {
        if (!this.walletClient)
            throw new Error("VeraxSDK - Wallet not available");
        try {
            const { request } = await this.web3Client.simulateContract({
