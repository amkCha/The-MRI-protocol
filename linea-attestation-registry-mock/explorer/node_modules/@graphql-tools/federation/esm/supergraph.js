import { buildASTSchema, isInterfaceType, Kind, parse, parseType, print, TypeInfo, visit, visitWithTypeInfo, } from 'graphql';
import { delegateToSchema, extractUnavailableFields, } from '@graphql-tools/delegate';
...
export function getFieldMergerFromSupergraphSdl(supergraphSdl) {
                    ...
                    for (const fieldNode of info.fieldNodes) {
                        ...
                        for (const candidate of candidatesReversed) {
                            ...
                            if (candidate.transformedSubschema) {
                                const unavailableFields = extractUnavailableFields(candidate.transformedSubschema.transformedSchema, typeFieldMap[candidate.fieldName], fieldNode, () => true);
...
export function getSubschemasFromSupergraphSdl({ supergraphSdl, onExecutor = ({ endpoint }) => buildHTTPExecutor({ endpoint }), batch = false, }) {
    ...
    const externalFieldMap = new Map();
    ...
    for (const [subgraphName, endpoint] of subgraphEndpointMap) {
        ...
        const fakeTypesIfaceMap = new Map();
        const iFaceTypeMap = new Map();
            ...
            if (interfaceInSubgraph.kind === Kind.INTERFACE_TYPE_DEFINITION) {
                ...
                for (const definitionNode of supergraphAst.definitions) {
                    if (definitionNode.kind === Kind.OBJECT_TYPE_DEFINITION &&
                        ...
                        isOrphan = false;
                        const existingType = subgraphTypes.find(typeNode => typeNode.name.value === definitionNode.name.value);
                        const iFaceNode = {
                            kind: Kind.NAMED_TYPE,
                            name: {
                                kind: Kind.NAME,
                        ...
                        if (existingType?.kind === Kind.OBJECT_TYPE_DEFINITION &&
                            !existingType.interfaces?.some(interfaceNode => interfaceNode.name.value === interfaceInSubgraph.name.value)) {
        ...
        function visitToReplaceTypeNames(data, nameMap) {
            ...
            transforms: [
                {
                    transformRequest(request, delegationContext, transformationContext) {
                        delegationContext.args = visitToReplaceTypeNames(delegationContext.args, fakeTypesIfaceMap);
                            ...
                            document: visit(request.document, visitWithTypeInfo(typeInfo, {
                                        ...
                                        if (isInterfaceType(parentType)) {
                                            ...
                                            for (const implementation of implementations) {
                                                ...
                                                if (externalFields?.has(node.name.value)) {
                                                    throw createGraphQLError(`Was not able to find any options for ${node.name.value}: This shouldn't have happened.`);
                                                }
                                            }
                                        }
                                    }
                                ...
                                [Kind.INLINE_FRAGMENT](node) {
                                    if (node.typeCondition) {
                                        const newTypeConditionName = fakeTypesIfaceMap.get(node.typeCondition?.name.value);
                                        if (newTypeConditionName) {
                                            transformationContext.replacedTypes ||= new Map();
                                            transformationContext.replacedTypes.set(newTypeConditionName, node.typeCondition.name.value);
                                            return {
                                                ...node,
                                                typeCondition: {
                                                    kind: Kind.NAMED_TYPE,
